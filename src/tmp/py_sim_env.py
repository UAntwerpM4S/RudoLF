...
from tools import create_checkpoints_from_simple_path, check_collision_ship
...
class PySimEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, time_step=0.1, max_steps=1500, verbose=None, target_pos=None, ship_pos=None, wind=False, current=False):
        ...
        self.initial_ship_pos = np.array([2098, -58])
    ...
    def env_specific_reset(self):
        """
        Environment-specific reset functionality.
        This method is called during reset and can be used for custom reset logic.
        """
        pass
    ...
    def _update_ship_dynamics(self, action, alpha=0.2):
        ...
        # Time constraints for gradual action changes
        MAX_RUDDER_RATE = 0.07  # Maximum change in rudder angle per time step
        ...
        # Smooth action application with heading consideration
        #heading = False
        #if self.heading_error < -0.1 or self.heading_error > 0.1:
        #    heading = True
    
        # Apply your existing logic with gradual actions
        if abs(target_rudder - self.current_action[0]) > 0.3:
            smoothed_action = [gradual_rudder, gradual_thrust]
        else:
            smoothed_action = [self.current_action[0], gradual_thrust] 
        #if abs(target_thrust - self.current_action[1]) > 0.2:
        #    smoothed_action = [smoothed_action[0], gradual_thrust]
        #else:
        #    smoothed_action = [smoothed_action[0], self.current_action[1]]
        ...
        r = np.clip(r + dr * self.time_step, self.MIN_YAW_RATE, self.MAX_YAW_RATE)
    ...
    def _calculate_heading_error(self, target_heading, current_heading):
    ...
    def _calculate_reward(self):
        ...
        # Distance to checkpoint (keep existing)
        ...
        # Cross-track error (keep existing but reduce weight)
