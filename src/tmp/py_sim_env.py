...
class PySimEnv(gym.Env):
    ...
    def _update_ship_dynamics(self, action, alpha=0.2):
        ...
        #smoothed_action = self._apply_pi_controller([round(action[0], 1), abs(round(action[1], 1))])
        heading = False
        if self.heading_error < -0.1 or self.heading_error > 0.1:
            heading = True
        if abs(action[0] - self.current_action[0]) > 0.2 and heading == True:
            smoothed_action = [action[0], abs(action[1])]
        else:
            smoothed_action = [self.current_action[0], abs(action[1])] 
