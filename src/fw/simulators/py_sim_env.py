...
MIN_MOVEMENT_THRESHOLD = 0.07
STUCK_STEPS_THRESHOLD = 40
PERP_LINE_PROXIMITY = 2.0
...
class PySimEnv(BaseEnv):
    ...
    YAW_RATE_DAMPING = YAW_RATE_DAMPING
    ...
    SHAPING_WINDOW = SHAPING_WINDOW
    DECAY_SCALE = DECAY_SCALE
    REWARD_DISTANCE_SCALE = REWARD_DISTANCE_SCALE
    REWARD_DIRECTION_SCALE = REWARD_DIRECTION_SCALE
    PENALTY_DISTANCE_SCALE = PENALTY_DISTANCE_SCALE
    CROSS_TRACK_ERROR_PENALTY_SCALE = CROSS_TRACK_ERROR_PENALTY_SCALE
    ...
    RUDDER_DEAD_ZONE = RUDDER_DEAD_ZONE
    THRUST_DEAD_ZONE = THRUST_DEAD_ZONE
    DERIVATIVE_FILTER_ALPHA = DERIVATIVE_FILTER_ALPHA
    MAX_RUDDER_RATE_CHANGE = MAX_RUDDER_RATE_CHANGE
    MAX_THRUST_RATE_CHANGE = MAX_THRUST_RATE_CHANGE
    ...
    STUCK_STEPS_THRESHOLD = STUCK_STEPS_THRESHOLD
    PERP_LINE_PROXIMITY = PERP_LINE_PROXIMITY
    ...
    def _calculate_reward(self) -> Tuple[float, bool]:
        ...
        if movement < self.MIN_MOVEMENT_THRESHOLD:
            self.stuck_steps += 1
            if self.stuck_steps > self.STUCK_STEPS_THRESHOLD:
                # previously was 'reward -= 0.6' and commented out penalty; keep same behavior but retain counter
                pass
        else:
            self.stuck_steps = 0
    ...
    def _is_near_perpendicular_line(self, checkpoint: dict) -> bool:
        ...
        return self._distance_from_point_to_line(self.ship_pos, line_start, line_end) <= self.PERP_LINE_PROXIMITY
