if __name__ == "__main__" or __name__ == "fw.main":
    ...
    envs = [get_environment(fh_sim_env_name)]
            # get_environment(py_sim_env_name)]
    ...
    if "train" == action:
        configuration = {py_sim_env_name: StopCondition(stop_criterion=Criterion.TARGET_REWARD, target_reward=1.01, patience=3, max_time_steps=600),
                         fh_sim_env_name: StopCondition(stop_criterion=Criterion.TARGET_REWARD, target_reward=0.86, patience=3, max_time_steps=800),
